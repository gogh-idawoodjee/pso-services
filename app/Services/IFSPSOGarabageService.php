...
// will refactor this thing to ResourceService sometime in the future when it decides to work


class IFSPSOGarabageService extends IFSService
{
    ...
    public function getResource($resource_id, $dataset_id, $base_url)
    {
    ...
    public function getResourceShiftsFormatted()
    {
        $this->getShifts();

        return collect($this->shifts)->map(function ($item, $key) {
    ...
    public function getResourceUtilization()
    {
        if (isset($this->pso_resource['Plan_Route'])) {
            if (isset($this->pso_resource['Plan_Route']['plan_id'])) {
                ...
                $this->utilization['dates'] = collect($this->pso_resource['Plan_Route'])->map(function ($item, $key) {
                    return ['date' => Carbon::create($item['shift_start_datetime'])->toFormattedDateString()];
                });
                $this->utilization['utilization'] = collect($this->pso_resource['Plan_Route'])->map(function ($item, $key) {
                    return ['utilization' => $item['utilisation']];
                });
                $this->utilization['travel'] = collect($this->pso_resource['Plan_Route'])->map(function ($item, $key) {
    ...
    public function getScheduleableResources($dataset_id, $token)
    {
        $schedule = new IFSPSOScheduleService(null,null,null,null,null,null,'cb847e5e-8747-4a02-9322-76530ef38a19');

        $overallschedule = collect($schedule->getSchedule($dataset_id)->collect());

        $resources = collect($overallschedule->get('Resources'));
        $shifts = collect($overallschedule->get('Plan_Route'))->groupBy('resource_id');
        $events = collect($overallschedule->get('Schedule_Event'));;
...
//        });
        ...
        if (!Arr::has($events, 'id')) {
            $events = collect($events)->mapToGroups(function ($item, $key) {
...
//        }
        ...
        $plans = collect($overallschedule->get('Plan_Resource'))->keyBy('resource_id');
        return collect($resources)->map(function ($item, $key) use ($events) {
        ...
        })->map(function ($item, $key) use ($shifts, $plans) {
            return collect($item)
                ->put('route', $plans[$item['id']])
                ->put('shiftcount', count($shifts[$item['id']]))
    ...
    public function setEvent($event_data, $resource_id): JsonResponse
    {
        ...
        // now we need to figure out if we need to auth or not // really this will have to be done at the controller to intitialize this instance of the service
    ...
    public function ScheduleEventPayloadPart($event_type, $resource_id)
    {
        return
            [
...
//        return $shift_set;
        ...
        $ram_rota_item_payload = $this->RAMRotaItemPayload(collect(collect($shift_set)->firstWhere('id', $shift_data->shift_id)), $shift_data->rota_id, $shift_data->turn_manual_scheduling_on, $shift_data->shift_type, "Manual Scheduling Only set to " . ($shift_data->turnmansched_on ? "ON" : "OFF") . " by the thingy tool.(" . Carbon::now()->toDateTimeString() . ")");
    ...
    private function apiResponse($code, $description, $payload)
    {
    ...
    {
        $sendRota = Http::withHeaders(['apiKey' => $token])
            ->post('https://' . $this->localmode ? $this->pso_environment->base_url : $base_url . '/IFSSchedulingRESTfulGateway/api/v1/scheduling/data',
    ...
    private function sendPayloadToPSO($payload, $token, $base_url)
    {
        ...
        if ($this->localmode) {
            $response = Http::timeout(5)
                ...
                ->connectTimeout(5)
                ->post('https://' . $this->pso_environment->base_url . '/IFSSchedulingRESTfulGateway/api/v1/scheduling/data', $payload);
        } else {
            $response = Http::timeout(5)
                ->withHeaders(['apiKey' => $token])
                ->connectTimeout(5)
    ...
    private function RAMRotaItemPayload($rawshift, $rota_id, $turnmansched_on, $shift_type, $description)
    {
        return [
            'id' => $rawshift->get('id'),
            'ram_rota_id' => "$rota_id",
            'manual_scheduling_only' => $turnmansched_on,
