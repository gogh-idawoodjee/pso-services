<div>
    <div class="card card-default m-t-20">
        <div class="card-header  separator">
            <div class="card-title">Edit <span>{{$env_data['name']}}</span>
            </div>
        </div>
        <div class="card-body  m-t-20">
            <div class="col-md-12">
                <form role="form" wire:submit.prevent="updateEnvironment">
                    <div class="row column-seperation">
                        <div class="col-lg-12">

                            <div class="row">
                                <div class="col-md-4">
                                    <div class="form-group form-group-default required">
                                        <label>Environment Name</label>
                                        <input type="text" class="form-control" name="name" required
                                               wire:model="env_data.name">
                                    </div>
                                </div>
                                <div class="col-md-4">
                                    <div class="form-group form-group-default required">
                                        <label>Username</label>
                                        <input type="text" class="form-control" name="username" required
                                               wire:model="env_data.username">
                                    </div>
                                </div>
                                <div class="col-md-4">
                                    <div class="form-group form-group-default required">
                                        <label>Password</label>
                                        <input type="password" class="form-control" name="password"
                                               wire:model="env_data.password">
                                    </div>
                                </div>


                            </div>

                            <div class="row">
                                <div class="col-md-6">
                                    <div class="form-group form-group-default required">
                                        <label>Base URL</label>
                                        <input type="url" class="form-control" name="base_url"
                                               wire:model="env_data.base_url">
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="form-group form-group-default required">
                                        <label>Account ID</label>
                                        <input type="text" class="form-control" name="account_id" required
                                               wire:model="env_data.account_id">
                                    </div>
                                </div>
                            </div>

                        </div>
                    </div>
                    <div class="pull-right">
                        <button aria-label="" type="button" class="btn btn-default btn-cons">Clear
                        </button>
                        <button aria-label="" type="submit" class="btn btn-primary btn-cons">
                            Update Environment
                        </button>
                    </div>
                </form>
                <div class="row">
                    @if($errors->count())
                    <div class="alert alert-warning" role="alert">
                        <button aria-label="" class="close" data-dismiss="alert"></button>
                                                <strong>Please check: </strong>
                        {{$errors->first()}}

                    </div>
                    @enderror


                </div>
            </div>
        </div>

    </div>

    <div class="card card-default m-t-20">
        <div class="card-header  separator">
            <div class="card-title">Add Dataset
            </div>
        </div>
        <div class="card-body  m-t-20">
            <div class="col-md-12">
                <form role="form" wire:submit.prevent="addDataset">
                    <div class="row column-seperation">
                        <div class="col-lg-12">

                            <div class="row">
                                <div class="col-md-6">
                                    <div class="form-group form-group-default required">
                                        <label>Dataset ID</label>
                                        <input type="text" class="form-control" name="dataset_id" required
                                               wire:model="dataset_data.dataset_id">
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="form-group form-group-default">
                                        <label>Rota ID</label>
                                        <input type="text" class="form-control" name="rota_id"
                                               wire:model="dataset_data.rota_id">
                                    </div>
                                </div>

                            </div>

                        </div>
                    </div>
                    <div class="pull-right">
                        <button aria-label="" type="button" class="btn btn-default btn-cons">Clear
                        </button>
                        <button aria-label="" type="submit" class="btn btn-primary btn-cons">
                            Add Dataset
                        </button>
                    </div>
                </form>
                <div class="row">
                    @error('env_data.account_id')
                    <div class="alert alert-warning" role="alert">
                        <button aria-label="" class="close" data-dismiss="alert"></button>
                        {{--                        <strong>Please check: </strong>{{$message}}--}}
                    </div>
                    @enderror


                </div>
            </div>
        </div>

    </div>
    @if(count($datasets))
        <div class="card card-default m-t-30">
            <div class="card-header ">
                <div class="card-title">Datasets
                </div>
                <div class="pull-right">
                    <div class="col-xs-12">
                        <input type="text" id="search-table" class="form-control pull-right" placeholder="Search">
                    </div>
                </div>
                <div class="clearfix"></div>
            </div>
            <div class="card-body">
                <table class="table table-hover demo-table-search table-responsive-block" id="tableWithSearch">
                    <thead>
                        <tr>

                            <th>Dataset ID</th>
                            <th>Rota ID</th>
                            <th>Last Update</th>

                            <th></th>
                        </tr>
                    </thead>
                    <tbody>
                        @foreach($datasets as $dataset)
                            <tr>
                                <td class="v-align-middle semi-bold">
                                    <p>{{$dataset->dataset_id}}</p>

                                </td>
                                <td class="v-align-middle">
                                    <p>{{$dataset->rota_id}}</p>
                                </td>
                                <td class="v-align-middle">
                                    <p>{{$dataset->updated_at}}</p>
                                </td>

                                <td class="v-align-middle">
                                    <div>
                                    <a href="" wire:click.prevent="deleteDataset('{{ $dataset->id }}')">
                                        <i class="pg-icon">bin</i></a>
                                    </div>
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
            </div>
        </div>
    @endif
</div>

